    ...
    <template #content>
      <div class="flex flex-col gap-y-2 mt-2">
        <div v-if="ride.minSizeWithEscort || ride.minSizeWithoutEscort">
          <h3 class="font-bold text-indigo-700 dark:text-indigo-300">{{ $t('ride.stats.lengthTitle', [ride.title]) }}</h3>
          <p>{{ sizeSentence }}</p>
        </div>

        <div v-if="ride.minAgeWithoutEscort || ride.minAgeWithEscort">
          <h3 class="font-bold text-indigo-700 dark:text-indigo-300">{{ $t('ride.stats.ageTitle', [ride.title]) }}</h3>
          {{ ageSentence }}
        </div>

        <div v-if="park">
          <h3 class="font-bold text-indigo-700 dark:text-indigo-300">{{ $t('ride.stats.locationTitle', [ride.title]) }}</h3>
          {{ locationSentence }}
        </div>
